...
input_file = '/Users/jasper/Desktop/HackerRank/PMLC/Ch02/adult.data.txt'
X = []
y = []
count_lessthan50k = 0
count_morethan50k = 0
...
with open(input_file, 'r') as f:
    for line in f.readlines():
        ...
        if data[-1] == '<=50K' and \
                count_lessthan50k < num_images_threshold:
            X.append(data)
            count_lessthan50k += 1
        elif data[-1] == '>50K' and \
        ...
        if count_lessthan50k >= num_images_threshold \
                and count_morethan50k >= num_images_threshold:
            break
...
for i, item in enumerate(input_data):
    if item.isdigit():
        input_data_encoded.append(input_data[i])
    else:
        input_data_encoded.append(label_encoder[i].transform(input_data[i]))
        count = count + 1
